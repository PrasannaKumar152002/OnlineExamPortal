...
public class LoginService {
	...
	public static Map<String, Object> checklogin(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			...
			GenericValue userLogin = EntityQuery.use(delegator).from("UserLogin").where("userLoginId", user).queryOne();
			if (userLogin != null) {
				System.out.println("Entered into loginuser");
				String password = userLogin.getString("currentPassword");
				System.out.println("Fetched password before decrypted :" + password);
